 ...
 * @since 1.0.0
 */
class Jet_Woo_Builder_Plugin_Changelog {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		if ( $plugin === $plugin_file ) {
			...
			$plugin_meta['view-details'] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
				...
				esc_attr( sprintf( __( 'More information about %s', 'jet-woo-builder' ), $this->api['name'] ) ),
				esc_attr( $this->api['name'] ),
				esc_html__( 'View details', 'jet-woo-builder' )
			);
...
if ( ! function_exists( 'jet_woo_builder_plugin_changelog' ) ) {
	function jet_woo_builder_plugin_changelog() {
		return new Jet_Woo_Builder_Plugin_Changelog();
	}
}
