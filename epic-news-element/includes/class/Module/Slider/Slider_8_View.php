...
Class Slider_8_View extends SliderViewAbstract
{
	public function content($results)
	{
		...
		foreach($results as $key => $post)
		{
			...
			$image = apply_filters('epic_single_image_owl', $post_thumbnail_id, 'epic-350x250');
		...
		if(!empty($result))
		{
			...
			$output =
				"<div {$this->element_id($attr)} class=\"jeg_slider_wrapper {$column_class} {$this->unique_id} {$this->get_vc_class_name()} {$attr['el_class']}\">
                    <div class=\"jeg_slider_type_8 jeg_slider epic-owl-carousel\" data-items=\"{$number_item}\" data-autoplay=\"{$attr['enable_autoplay']}\" data-delay=\"{$autoplay_delay}\">
                        {$content}
                    </div>
                </div>";
			return $output;
