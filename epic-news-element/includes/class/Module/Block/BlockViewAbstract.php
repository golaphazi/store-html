...
abstract Class BlockViewAbstract extends ModuleViewAbstract
{
    public function render_module($attr, $column_class)
    {
	    ...
	    $name           = str_replace('epic_block_','',$this->class_name);
        $style_output   = epic_header_styling($attr, $this->unique_id . ' ');
        $style_output   .= epic_module_custom_color($attr, $this->unique_id . ' ', $name);
    ...
    {
        return apply_filters('epic_module_block_container_extend_before', '', $attr);
    }

    public function get_content_after($attr)
    {
        return apply_filters('epic_module_block_container_extend_after', '', $attr);
    }

    public function get_navigation_before($attr)
    {
        return apply_filters('epic_module_block_navigation_extend_before', '', $attr);
    }

    public function get_navigation_after($attr)
    {
        return apply_filters('epic_module_block_navigation_extend_after', '', $attr);
    }
    ...
    public function render_navigation($attr, $next = false, $prev = false, $total_page = 1)
    {
        ...
        if($attr['pagination_mode'] === 'nextprev')
        {
            ...
            if ($attr['pagination_nextprev_showtext']) {
                ...
                $prev_text = "<i class=\"fa fa-angle-left\"></i> ". esc_html__("Prev", 'epic-ne');
                $next_text = esc_html__("Next", 'epic-ne') . "  <i class=\"fa fa-angle-right\"></i>";
            }

            $output =
                "<div class=\"jeg_block_nav {$additional_class}\">
                    <a href=\"#\" class=\"prev {$prev}\" title=\"". esc_html__("Previous", 'epic-ne') ."\">{$prev_text}</a>
                    <a href=\"#\" class=\"next {$next}\" title=\"". esc_html__("Next", 'epic-ne') ."\">{$next_text}</a>
                </div>";
        }
        ...
        if($attr['pagination_mode'] === 'loadmore' || $attr['pagination_mode'] === 'scrollload')
        {
            ...
            $output =
                "<div class=\"jeg_block_loadmore {$additional_class}\">
                    <a href=\"#\" class='{$next}' data-load='" . esc_html__('Load More', 'epic-ne') . "' data-loading='" . esc_html__('Loading...', 'epic-ne') . "'> " . esc_html__('Load More', 'epic-ne') . "</a>
                </div>";
        }
        ...
        $output = apply_filters('epic_module_block_search_navigation', $output, $attr, $next, $prev, $total_page);
    ...
    public function render_normal_navigation($args, $total, $current = 1)
    {
        ...
        $defaults = array(
            ...
            'show_all' => false,
            'prev_next' => true,
            'prev_text' => esc_html__('Prev', 'epic-ne'),
        ...
        switch ( $args['type'] ) {
            ...
            default :
                ...
                $paging_text = sprintf(__('Page %s of %s', 'epic-ne'), $current, $total);
    ...
    public function render_header($attr)
    {
        if ( defined('POLYLANG_VERSION')  && defined( 'PLL_INC' )  )
        {
            ...
            $attr['first_title'] = pll__($attr['first_title']);
            $attr['second_title'] = pll__($attr['second_title']);
            $attr['header_filter_text'] = pll__($attr['header_filter_text']);
        }
        ...
        {
            $heading_icon   = epic_header_icon($attr['header_icon']);
