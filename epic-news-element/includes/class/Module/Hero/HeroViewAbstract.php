...
use EPIC\Image\ImageBackgroundLoad;
...
abstract Class HeroViewAbstract extends ModuleViewAbstract
{
    ...
    public function generate_style($attr)
    {
        ...
        for($i = 1; $i <= $this->get_number_post(); $i++)
        {
            ...
            {
            	$degree = isset($attr['hero_item_' . $i . '_degree']['size']) ? $attr['hero_item_' . $i . '_degree']['size'] : $attr['hero_item_' . $i . '_degree'] ;

                if($attr['hero_style'] === 'jeg_hero_style_5') {
                    $style .=
                        ".jeg_heroblock.{$this->unique_id} .jeg_hero_item_{$i} .jeg_thumb a > div:after {
                            background: -moz-linear-gradient({$degree}deg, {$attr['hero_item_' . $i . '_start_color']} 0%, {$attr['hero_item_' . $i . '_end_color']} 100%);
                            background: -webkit-linear-gradient({$degree}deg, {$attr['hero_item_' . $i . '_start_color']} 0%, {$attr['hero_item_' . $i . '_end_color']} 100%);
                            background: linear-gradient({$degree}deg, {$attr['hero_item_' . $i . '_start_color']} 0%, {$attr['hero_item_' . $i . '_end_color']} 100%);
                        }";
                } else {
                    $style .=
    ...
    public function data_attr($attr)
    {
	    ...
	    if ( isset($attr['hero_slider_enable']) && $attr['hero_slider_enable'] )
	    {
		    $slider_delay = isset($attr['hero_slider_delay']['size']) ? $attr['hero_slider_delay']['size'] : $attr['hero_slider_delay'];

		    $output .= isset($attr['hero_slider_auto_play']) && $attr['hero_slider_auto_play'] ? ' data-autoplay="' . $attr['hero_slider_auto_play'] . '""' : '';
		    $output .= $slider_delay ? ' data-delay="' . $slider_delay . '""' : '';
	    }
    ...
    public function render_module($attr, $column_class)
    {
	    ...
	    if ( isset($attr['hero_slider_enable']) && $attr['hero_slider_enable'] )
	    {
	    	$slider_item = isset($attr['hero_slider_item']['size']) ? $attr['hero_slider_item']['size'] : $attr['hero_slider_item'];

		    $attr['number_post'] = $attr['number_post'] * $slider_item;
	    }
