wp.customize.controlConstructor['jeg-typography'] = wp.customize.controlConstructor.default.extend({
	...
	ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( jegAllFonts, function( font, i ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.jeg-' + sub + '-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.jeg-' + sub + '-wrapper' ).css( 'display', 'block' );
				}
		...
		picker = this.container.find( '.jeg-color-control' );
