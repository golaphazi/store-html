...
Class Image
{
    ...
    {
        $generate_image = epic_get_option('image_generator', 'normal');
    ...
    {
        $mechanism = epic_get_option('image_load', 'lazyload');
        ...
        add_filter( 'epic_image_lazy_owl' , array($image, 'owl_lazy_image'), null, 2);
        add_filter( 'epic_single_image_lazy_owl' , array($image, 'owl_lazy_single_image'), null, 2);
        add_filter( 'epic_image_thumbnail' , array($image, 'image_thumbnail'), null, 2);
        add_filter( 'epic_image_thumbnail_unwrap' , array($image, 'image_thumbnail_unwrap'), null, 2);
        add_filter( 'epic_single_image_unwrap' , array($image, 'single_image_unwrap'), null, 2);
        add_filter( 'epic_single_image_owl' , array($image, 'owl_single_image'), null, 2);

        add_filter( 'epic_single_image', array($image, 'single_image'), null, 3 );
    ...
    public function custom_size($sizes)
    {
        ...
        foreach($this->image_size as $key => $size) {
            $sizes[$key] = esc_html__('Custom Size', 'epic-ne');
        }
        return $sizes;
    }
    ...
    public static function generate_image_retina($image, $imageretina, $alt, $echo)
    {
        ...
        if($echo) {
            echo jeg_sanitize_output( $header_logo );
        } else {
            return $header_logo;
        }
    }
